...
mod test {
    ...
    async fn test_format_translated_result() -> bool {
        let mock = crate::mock::Mock::new();
        let formatter = crate::formatter::Formatter::format(0, &mock.text_splitter.split_result);
        let translator = crate::translator::Translator::new();
        let source_language = "en".to_string();
    ...
    async fn test_format_translated_result_multiple_times() {
        ...
        for _ in 0..total_runs {
            handles.push(test_format_translated_result());
        }
        ...
        // 如果你想在成功率低于某个阈值时让测试失败
        assert!(
            success_rate >= 50.0,
            "Success rate is below the acceptable threshold"
        );
    }
}
